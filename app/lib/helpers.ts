...
import { ShopPolicies } from "app/integrations/shopify/types/Policies";
...
export function convertOrdersToText(orders:Order[]) {
  ...
  function buildFullProductText(product: Product, shopDomain: string): string {
    ...
    const description = product.description
      ? removeHtmlAndLiquidTags(product.description)
      : "";
    ...
    if (product.metafields?.edges?.length) {
      metafieldsText = product.metafields.edges
        .map((mf) => {
          ...
          val = removeHtmlAndLiquidTags(val);
          return `  - ${key}: ${val}`;
        })
        .join("\n");
    }
 ...
 * HTMLタグとLiquidタグを除去
 */
export function removeHtmlAndLiquidTags(text: string): string {
  return text
    // HTMLタグを除去
    .replace(/<[^>]*>/g, "")
    // Liquid制御タグ（{% ... %}）を除去
 ...
 */
   ...
   */
  export function convertPoliciesToText(shopPolicies: ShopPolicies): string {
    return shopPolicies
    .map((p: any) => `${p.title}${removeHtmlAndLiquidTags(p.body)}${CHUNK_SEPARATOR_SYMBOL}`)
    .join("\n")
    .replace(/\\n/g, "\n")
    .replace(/&nbsp;/g, "")
    .trim();

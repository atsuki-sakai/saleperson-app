 ...
 * （商品数が多いと時間がかかります）
 */
export async function convertProductToText(product: Product, shop: string) {
  const productText = buildFullProductText(product, shop);

  // 要約呼び出し
  let summary = "";
  try {
    console.log("要約を開始しました");
    const response = await generateSummaryAndTargets(productText);
    summary = transformDifyResponseToString(response);
  ...
  const summaryText = summaryObjectToText(summaryObj);

  const productResult = productText + summaryText;

  // 最終的に CHUNK_SEPARATOR_SYMBOL で区切って返す
 ...
 * Dify API呼び出し
 */
export async function generateSummaryAndTargets(text: string) {
  try {
    const response = await fetch(`${DIFY_BASE_URL}/chat-messages`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${DIFY_GENERATE_SUMMARY_AND_TARGETS_API_KEY}`,
      },
      body: JSON.stringify({
        inputs: {},
        query: text,
    ...
    console.error("Fetch error (Dify):", error);
    throw error;
  }
}

/**
 * レスポンス(answer: ```json ... ```)を見やすく整形
 */
...
/**
 * Difyなどで返却される要約JSONを、改行区切りのテキストへ変換する関数。
 *
 * 以下のようなフィールド構造を想定：
 * {
 *   "productInfo": {...},
 ...
 * }
 *
 * @param summaryObj 要約のJSONオブジェクト
 * @returns 整形したテキスト
 */
export function summaryObjectToText(summaryObj: any): string {
